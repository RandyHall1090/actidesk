...
**Confirm email — still not actually off**: Randy turned off Supabase's "Confirm email" setting (2026-09-08). Re-tested live with a fresh signup right after: `signUp()` succeeded (an `auth.users` row was created) but returned no session, and `email_confirmed_at` stayed `null` — the exact signature of email confirmation still being required. Either a different setting was changed, the change didn't save, or there's a second toggle also gating this (Supabase has both a project-level "Confirm email" and provider-level settings). Needs Randy to re-check Authentication → Sign In / Up → Email in the dashboard and confirm which toggle is actually controlling this.
